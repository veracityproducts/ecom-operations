...
class ShippoService:
    ...
    def __init__(self, api_token: str, rate_limit_tier: str = "standard"):
        self.client = ShippoClient(api_token, rate_limit_tier=rate_limit_tier)
        self.logger = logging.getLogger(__name__)
    ...
    async def get_rates_and_create_label(
        ...
        async with self.client as client:
            ...
            return await self.create_label_with_known_rate(
                shipment_data, 
        ...
        try:
            ...
            return {
                ...
                "label": {
                    ...
                    "rate_amount": label_result.rate.get("amount", "0.00"),
                    "carrier": label_result.rate.get("provider", "")
                },
                "packing_slip": {
                    "url": packing_slip_result.packing_slip_url,
                    "expires_at": packing_slip_result.expires_at.isoformat()
                },
                "total_cost": float(label_result.rate.get("amount", "0.00")) + 0.05,  # Label + $0.05 packing slip
